...
def Keccak_F(state):
	...
	for rnd in range(24):
		...
		for i in range(5):
			for j in range(5):
				BB[j + 5 * ((2 * i + 3 * j) % 5)] = rotate_left(state[i + 5 * j], ROTL_table[i + 5 * j])
...
# 吸收和挤压函数
def sponge_func(input_data, dgstLen, bit_rate, delimited_suffix):
	if not dgstLen % 8 == 0:
		raise ValueError("digest length must be a multiple of 8")
	if dgstLen > 1600:
		raise ValueError("Length limit for digest is 1600 bits")
...
def SHA3(input_data, dgstLen):
	...
	return sponge_func(input_data, dgstLen, 1600 - (dgstLen * 2), 0x06)

# SHAKE
def SHAKE(inputData, variant, dgstLen):
	if not variant in (128, 256):
		raise ValueError("variant must be 128 or 256")

	return sponge_func(input_data, dgstLen, 1600 - (variant * 2), 0x1F)
...
if __name__ == "__main__":
	...
	print(f"\nSHAKE128值（输出128位）：{SHAKE(input_data, 128, 128).hex()}")
	print(f"\nSHAKE128值（输出256位）：{SHAKE(input_data, 128, 256).hex()}")
	print(f"\nSHAKE256值（输出256位）：{SHAKE(input_data, 256, 256).hex()}")
	print(f"\nSHAKE256值（输出512位）：{SHAKE(input_data, 256, 512).hex()}\n")
...
SHAKE：
SHAKE(<原始消息：字节>, <变体：正整数>, <摘要长度：正整数>)
*SHAKE的变体必须为128或256
*SHAKE的摘要长度必须为8的倍数（最大1600）
*SHAKE不支持HMAC
"""